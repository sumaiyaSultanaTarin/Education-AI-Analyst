...
Ported from dev_saif's fix (found live-testing the merged app): a full
browser reload opens a fresh WebSocket connection, which wipes
st.session_state — losing the active session client-side even though it's
still safe on the backend (api/session_store.py). The URL survives a
reload, so every page keeps ?session=<id> in its own URL and restores from
it if session_state comes back empty.
"""
